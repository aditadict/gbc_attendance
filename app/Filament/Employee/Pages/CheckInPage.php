...
class CheckInPage extends Page
{
    ...
    public function getBranchMapData(): array
    {
        $b = $this->getEmployee()?->branch;
        if (! $b) return [];
        return [
    ...
    public function checkIn(float $lat = 0, float $lng = 0): void
    {
        $employee = $this->getEmployee();
    ...
    public function checkOut(float $lat = 0, float $lng = 0): void
    {
        ...
        $attendance->update([
            'check_out_at'       => now(),
            'check_out_latitude'  => $lat ?: null,
            'check_out_longitude' => $lng ?: null,
        ]);
